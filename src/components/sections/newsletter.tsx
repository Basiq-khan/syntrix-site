export default function NewsletterBanner() {
  return (
    <section className="w-full px-4 py-16 sm:py-24 md:py-36 sm:px-10 md:px-16 lg:px-24"
      style={{ background: "url('/885c187d3a8a6f56d114d0a8d2481bf15c7a1d0a.jpg') center/cover no-repeat" }}
    >
      <div className="max-w-[1400px] mx-auto">

        {/* Main Dark Container Box */}
        <div className="w-full bg-[#000219] rounded-[24px] overflow-hidden grid grid-cols-1 lg:grid-cols-12 items-center relative min-h-[320px] sm:min-h-[360px]">

          {/* ========================================================= */}
          {/* LEFT COLUMN: Large Decorative Star / Circle Badge          */}
          {/* ========================================================= */}
          <div className="col-span-1 lg:col-span-4 h-full items-center justify-start relative overflow-hidden min-h-[200px] lg:min-h-auto hidden md:flex">
            {/* Circular frame with thick lime border and star element */}
            <div className="absolute left-1/2 -translate-x-1/2 lg:left-[-50px] lg:translate-x-0 w-[240px] h-[240px] sm:w-[320px] sm:h-[320px] rounded-full border-[3px] border-[#D4FF00] flex items-center justify-center bg-transparent">

              {/* Lime Green Cutout Star Graphic */}
              <div className="w-[120px] h-[120px] sm:w-[180px] sm:h-[180px] text-[#D4FF00] relative lg:left-4">
          ...
          <div className="col-span-1 lg:col-span-8 flex flex-col text-left px-6 py-8 sm:p-12 lg:pl-0 lg:pr-16 z-10">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-full flex flex-col sm:flex-row items-end sm:items-center relative">
              ...
              <button
                type="submit"
                className="w-full sm:w-auto shrink-0 bg-transparent text-white border border-[#FF5533] font-['Anton'] font-normal text-[14px] tracking-widest uppercase px-10 py-3 mt-6 sm:mt-0 sm:-ml-px transition-all duration-300 hover:bg-[#FF5533] hover:text-black"
              >
                SUBSCRIBE NOW
              </button>
